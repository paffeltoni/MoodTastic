...
class PostDB {
    ...
    public static function updatePost($id, $title, $body, $thumbnail, $categoryID, $isFeatured) {
        $db = Blog_Database::getDB();

        $query = 'UPDATE posts 
              SET title = :title, body = :body, category_id = :category_id, is_featured = :is_featured';

        // Update the thumbnail only if a new thumbnail was uploaded
        if ($thumbnail['name']) {
